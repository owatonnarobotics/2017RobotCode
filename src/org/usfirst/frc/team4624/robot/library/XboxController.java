 ...
 * @version 2015
 * 
 *          This class wraps around the Joystick class in order to make working with Xbox360 controllers less of a pain.
 * 
 *          The values from this class can be used in two ways. One could either check each Button every cycle with .get(), or they could
 *          call commands directly from the Buttons with .whenPressed()
 * 
 *          USAGE: // Initialization myXboxController = new XboxController( <port the controller is on (starts at 0)> );
 *          myXboxController.leftStick.setThumbstickDeadZone( .2 ); // Optional. See code below for defaults.
 * 
 *          // Using buttons myXboxController.a.whenPressed( new MyCommand() ); myXboxController.lb.toggleWhenPressed( new MyCommand() );
 *          myXboxController.rightStick.whenPressed( new MyCommand() );
 * 
 *          // Getting values directly if( myXboxController.leftStick.getY() > .4 ) ...
 * 
 *          // Support of legacy methods (NOTE: These values are straight from the Joystick class. No deadzone stuff or anything) if(
 *          xboxController.getX() > .4 ) ...
 * 
 *          NOTES: Although I have confidence that this will work, not everything has been tested. This should work for the 2015 WPILib. The
 *          mappings of axis's and buttons may change in later years. I am not a good Java programmer.
 */
public class XboxController extends Joystick {
	...
	/**
	 * (Constructor #1) There are two ways to make an XboxController. With this constructor, you can specify which port you expect the
	 * controller to be on.
	 * 
	 * @param port
	 */
	...
	/**
	 * (Constructor #2) This is the other constructor. I would recommend using this one instead as it is unlikely that anything else but the
	...
	/**
	 * Rather than use an integer (which might not be what we expect) we use an enum which has a set amount of possibilities.
	 */
	...
	/**
	 * This is the relation of direction and number for .getPOV() used in the DirectionalPad class.
	 */
	...
	/**
	 * This class is used to represent one of the two Triggers on an Xbox360 controller.
	 */
	...
	/**
	 * turns this |--1--2--3--4--5--| into this ______|-1-2-3-4-5-|
	 * 
	 * @param input
	 * @param deadZoneSize
